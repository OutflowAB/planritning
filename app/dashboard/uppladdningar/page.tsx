...
export default function UppladdningarPage() {
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-4rem)] w-full items-center justify-center bg-[#f5f3f0] px-6 py-10">
      <div className="w-full max-w-4xl rounded-none border border-[#d8d2c8] bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="shrink-0 rounded-none border border-[#d8d2c8] bg-[#f7f4ef] px-3 py-2 text-sm font-semibold text-[#4d463f] transition hover:bg-[#eee8df]"
          >
            Ladda upp
          </button>
        </div>
              ...
              {uploads.map((upload) => {
                ...
                >
                  <div className="flex items-center justify-between gap-3 border-b border-[#e8e2d8] bg-[#f7f4ef] px-4 py-3">
                    ...
                    {isPreviewReady ? (
                      <Link
                        href={`/dashboard?imagePath=${encodeURIComponent(upload.file_path)}&imageName=${encodeURIComponent(upload.file_name)}${upload.preview_url ? `&imagePreviewUrl=${encodeURIComponent(upload.preview_url)}` : ""}`}
